...
4. 你的代理被配置为覆盖（而非追加）`X-Forwarded-For` 头

当 `ForwardedHeaderFilter` 正确配置时，`request.getRemoteAddr()` 将自动从转发头返回正确的客户端 IP。没有此配置，直接使用 `request.getRemoteAddr()` — 它返回直接连接的 IP，这是唯一可信任的值。
...
- 通过 `@NonNull` 和 `Optional` 适当强制空安全

**记住**：保持控制器精简、服务专注、仓库简单、错误集中处理。优化可维护性和可测试性。
