...
description: "使用 Plankton 的写入时代码质量强制执行 —— 通过钩子在每次文件编辑时自动格式化、lint 和 Claude 驱动的修复。"
origin: community
---
...
- 你希望在每次文件编辑时自动格式化和 lint（不仅是在提交时）
- 你需要防御智能体修改 linter 配置以通过检查而非修复代码
- 你想要分层模型路由来修复（Haiku 用于简单样式、Sonnet 用于逻辑、Opus 用于类型）
...
1. **PreToolUse 钩子** —— `protect_linter_configs.sh` 在编辑发生之前阻止对所有 linter 配置的修改
2. **Stop 钩子** —— `stop_config_guardian.sh` 在会话结束时通过 `git diff` 检测配置变更
3. **受保护文件列表** —— `.ruff.toml`、`biome.json`、`.shellcheckrc`、`.yamllint`、`.hadolint.yaml` 等

### 包管理器强制执行

Bash 上的 PreToolUse 钩子阻止遗留包管理器：
- `pip`、`pip3`、`poetry`、`pipenv` -> 已阻止（使用 `uv`）
- `npm`、`yarn`、`pnpm` -> 已阻止（使用 `bun`）
...
2. 添加 Plankton 钩子用于写入时代码质量强制执行
3. 使用 AgentShield 进行安全审计
4. 使用 ECC 的验证循环作为 PR 前的最终门控
...
**关键设置：**
- 禁用你不使用的语言以加速钩子
- `volume_threshold` —— 违规数超过此计数自动升级到更高的模型层级
...
- TypeScript/JavaScript：首选 Biome，Prettier 回退
