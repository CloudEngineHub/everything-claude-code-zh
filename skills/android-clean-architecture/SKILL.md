...
- 构建Android 或 KMP 项目模块
- 实施 UseCases、Repositories 或 DataSources
- 设计层之间的数据流（域、数据、展示）
...
- 向 UI 层暴露数据库实体或 DTO — 始终映射到域模型
- 将业务逻辑放入 ViewModels — 提取到 UseCases
- 使用 `GlobalScope` 或非结构化协程 — 使用 `viewModelScope` 或结构化并发
- 肥存储库实现 — 拆分为专注的 DataSources
- 循环模块依赖 — 如果 A 依赖于 B，B 不得依赖于 A

## 参考资料

参见技能：`compose-multiplatform-patterns` 以获取 UI 模式。
