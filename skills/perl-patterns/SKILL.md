---
name: perl-patterns
description: 现代 Perl 5.36+ 惯用写法、最佳实践和约定，用于构建健壮、可维护的 Perl 应用程序。
origin: ECC
---

# 现代 Perl 开发模式

惯用的 Perl 5.36+ 模式和最佳实践，用于构建健壮、可维护的应用程序。
...
将这些模式作为对现代 Perl 5.36+ 默认设置的偏好来应用：签名、显式模块、聚焦错误处理和可测试边界。下面的示例旨在作为起点复制，然后针对你面前的实际应用、依赖栈和部署模型进行细化。
...
优先使用 Moo 进行轻量级现代面向对象编程。仅当需要 Moose 的元协议时才使用 Moose。
...
# 好的做法：带 autodie 的三参数 open（核心模块，消除 'or die'）
use autodie;
...
# 不好的做法：两参数 open（shell 注入风险，参见 perl-security）
...
# 2. 间接对象语法（歧义解析）
my $obj = new Foo(bar => 1);            # 不好的做法
my $obj = Foo->new(bar => 1);           # 好的做法

# 3. 过度依赖 $_
map { process($_) } grep { validate($_) } @items;  # 难以跟踪
my @valid = grep { validate($_) } @items;           # 更好：拆分
...
# 5. 全局变量作为配置
our $TIMEOUT = 30;                       # 不好的做法：可变全局变量
use constant TIMEOUT => 30;              # 更好：常量
# 最好的做法：带默认值的 Moo 属性

# 6. 字符串 eval 加载模块
eval "require $module";                  # 不好的做法：代码注入风险
eval "use $module";                      # 不好的做法
use Module::Runtime 'require_module';    # 好的做法：安全的模块加载
require_module($module);
```

**记住**：现代 Perl 是干净、可读和安全的。让 `use v5.36` 处理样板代码，使用 Moo 进行面向对象编程，优先使用 CPAN 经过实战检验的模块而非手工编写的解决方案。
