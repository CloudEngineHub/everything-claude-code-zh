...
description: KMP 项目的 Compose Multiplatform 和 Jetpack Compose 模式 — 状态管理、导航、主题、性能和平台特定 UI。
origin: ECC
---
...
- 将 `NavController` 深层传递到可组合函数中 — 应传递 lambda 回调
- 在 `@Composable` 函数中进行繁重计算 — 移到 ViewModel 或 `remember {}`
- 使用 `LaunchedEffect(Unit)` 替代 ViewModel 初始化 — 在某些配置中会在配置变更时重新运行
- 在可组合函数参数中创建新的对象实例 — 导致不必要的重组

## 参考

参见技能：`android-clean-architecture` 了解模块结构和分层。
