...
description: 前端开发模式 — React、Next.js、状态管理、性能优化和 UI 最佳实践。
---
...
前端示例应使用合成或通用领域数据。不要收集、记录、持久化或显示凭据、访问令牌、SSN、健康数据、支付详情、私人电子邮件、电话号码或其他敏感个人数据，除非用户明确请求具有适当验证、编辑和访问控制的范围实现。

避免在未经明确批准的情况下添加分析、跟踪像素、第三方脚本或外部数据接收器。处理用户数据时，优先使用最小权限 API、记录前的客户端编辑，以及每个边界的服务端验证。
