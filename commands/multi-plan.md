...
等待增强后的提示，**用增强结果替换原始 $ARGUMENTS 用于所有后续阶段**。

**如果 ace-tool MCP 不可用**：跳过此步骤，所有后续阶段使用原始 `$ARGUMENTS`。
...
将**原始需求**（无预设偏见）分发给两个模型：

1. **Codex 后端分析**：
   - ROLE_FILE：`~/.claude/.ccg/prompts/codex/analyzer.md`
   - 重点关注：技术可行性、架构影响、性能考量、潜在风险
   - OUTPUT：多视角方案 + 优缺点分析

2. **Gemini 前端分析**：
   - ROLE_FILE：`~/.claude/.ccg/prompts/gemini/analyzer.md`
   - 重点关注：UI/UX 影响、用户体验、视觉设计
   - OUTPUT：多视角方案 + 优缺点分析
...
为降低 Claude 综合计划中遗漏的风险，可并行让两个模型输出"计划草稿"（仍**不允许**修改文件）：

1. **Codex 计划草稿**（后端权威）：
   - ROLE_FILE：`~/.claude/.ccg/prompts/codex/architect.md`
   - OUTPUT：逐步计划 + 伪代码（关注：数据流/边界情况/错误处理/测试策略）

2. **Gemini 计划草稿**（前端权威）：
   - ROLE_FILE：`~/.claude/.ccg/prompts/gemini/architect.md`
   - OUTPUT：逐步计划 + 伪代码（关注：信息架构/交互/可访问性/视觉一致性）

使用 `TaskOutput` 等待两个模型的完整结果，记录关键差异。
